...
load_dotenv(os.path.join(_PROJECT_ROOT, '.env.production'))
...
def valuation_bucket(pe_log_ratio_vs_sector, pe_ratio_fallback=None):
    """Prefer the per-prediction point-in-time log-ratio (from
    prediction_details.pe_log_ratio_vs_sector) when available. That column is
    log(stock_PE / sector_median_PE), so >0 means priced above sector median.
    Thresholds: >+0.5 (~65% above sector) → overvalued, <-0.5 (~40% below)
    → undervalued. Falls back to raw pe_ratio for live (non-backtest) rows
    that don't have the log-ratio populated."""
    if pe_log_ratio_vs_sector is not None:
        if pe_log_ratio_vs_sector > 0.5:
            return 'overvalued'
        if pe_log_ratio_vs_sector < -0.5:
            return 'undervalued'
        return 'fair_value'
    if pe_ratio_fallback is None or pe_ratio_fallback <= 0:
        return 'unknown'
    if pe_ratio_fallback < 15:
        return 'undervalued'
    if pe_ratio_fallback > 25:
        return 'overvalued'
    return 'fair_value'
...
def fetch_rows(conn, model_version: str | None):
    ...
    cols += ['s.market_cap', 's.pe_ratio', 'pd.pe_log_ratio_vs_sector']
...
def main():
    ...
    for h in HORIZONS:
        ...
        for r in rows:
            ...
            if not r[f'resolved_{h}'] or r[f'direction_correct_{h}'] is None:
                continue
            bucket = valuation_bucket(r.get('pe_log_ratio_vs_sector'), r.get('pe_ratio'))
            segs.setdefault(bucket, []).append(r[f'direction_correct_{h}'])
        results = [
            s for b, vals in sorted(segs.items())
            if (s := summarize(b, vals, f'direction_correct_{h}', args.min_runs))
