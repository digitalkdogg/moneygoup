...
export default function SellModal({ stock, onClose }: SellModalProps) {
  ...
  const recalcGain = (sharesVal: string, priceVal: string, type: 'all' | 'partial') => {
    const sharesNum = type === 'all' ? maxShares : parseFloat(sharesVal);
    const priceNum = parseFloat(priceVal);
    if (!isNaN(sharesNum) && !isNaN(priceNum) && sharesNum > 0 && priceNum > 0) {
      setRealizingGain((priceNum - parseFloat(stock.purchase_price as any)) * sharesNum);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (sellType === 'partial') {
      ...
      if (sharesNum > maxShares) { setError(`Cannot sell more than ${formatNumber(maxShares, 4)} shares`); return; }
    }
    setLoading(true);
    try {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 id="modal-title" className="text-base font-bold text-gray-900">Sell Shares</h2>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" id="sell-form">
            <fieldset className="space-y-2">
              <legend className="text-sm font-medium text-gray-700">Sell Type</legend>
              <div className="space-y-1.5">
                <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                  <input
                    type="radio" name="sellType" value="all"
                    checked={sellType === 'all'}
                    onChange={() => handleSellTypeChange('all')}
                    className="focus-ring"
                  />
                  Sell All ({maxShares.toFixed(4)} shares)
                </label>
                <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                  <input
                    type="radio" name="sellType" value="partial"
            ...
            {sellType === 'partial' && (
              <div>
                ...
                <input
                  id="sell-shares"
                  type="number" step="0.0001" min="0" max={maxShares}
                  value={shares}
                  onChange={(e) => { setShares(e.target.value); recalcGain(e.target.value, price, sellType); }}
            ...
            <div>
              ...
              <input
                ...
                value={price}
                onChange={(e) => { setPrice(e.target.value); recalcGain(shares, e.target.value, sellType); }}
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
            ...
            {realizingGain !== null && (
              <div className={`rounded-xl p-4 space-y-1.5 ${realizingGain >= 0 ? 'bg-green-50' : 'bg-red-50'}`} aria-live="polite">
                <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={`font-semibold ${realizingGain >= 0 ? 'text-green-700' : 'text-red-600'}`}>
            ...
            {sellType === 'all' && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3">
                <p className="text-sm text-yellow-800 font-medium">⚠️ This will close your position for {stock.symbol}</p>
              </div>
            )}
